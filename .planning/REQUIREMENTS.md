...
- [x] **INFRA-06**: Todos los movimientos pasan por una clase Executor con flag dry_run centralizado
- [x] **INFRA-07**: Los destinos nunca sobreescriben archivos existentes — sufijo (2), (3)... via _free_path()
- [x] **INFRA-08**: Los logs de cada ejecución se guardan en <unidad>\_organizer_logs\ con encoding UTF-8

### Menú

- [x] **MENU-01**: El script presenta un menú numerado en terminal al iniciar (sin argumentos CLI)
- [x] **MENU-02**: Desde el menú se puede activar modo dry-run antes de ejecutar cualquier operación
- [x] **MENU-03**: Al finalizar cualquier operación se muestra un resumen: archivos procesados, movidos, saltados, errores

### Renombrado

- [x] **RENAME-01**: El usuario puede aplicar renombrados desde un rename_plan.tsv existente (columnas old_path / new_path, separador tab)
- [x] **RENAME-02**: El aplicador de renombrados usa rutas literales (sin interpretar comodines)
- [x] **RENAME-03**: El script puede generar automáticamente un rename_plan.tsv sugerido escaneando la unidad y detectando nombres mal formateados
- [x] **RENAME-04**: El TSV generado se guarda en _organizer_logs\ para que el usuario lo revise antes de aplicar (nunca se auto-aplica)
...
- [x] **UNDO-01**: Cada operación que mueve o renombra archivos queda registrada en un log JSON atómico
- [x] **UNDO-02**: El log JSON guarda rutas relativas a la unidad más el número de serie del volumen (para sobrevivir cambios de letra de unidad)
- [x] **UNDO-03**: Desde el menú el usuario puede revertir la última ejecución completa

### Coherencia

- [x] **COH-01**: El script puede escanear la unidad y reportar vídeos fuera de las carpetas esperadas (Series, Peliculas)
- [x] **COH-02**: El reporte detecta series con episodios sin carpeta de temporada asignada
...
| UNDO-03 | Phase 3 | Complete |
| RENAME-03 | Phase 4 | Complete |
| RENAME-04 | Phase 4 | Complete |
